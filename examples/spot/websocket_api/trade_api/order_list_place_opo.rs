...
use binance_sdk::spot::{
    SpotWsApi,
    websocket_api::{
        OrderListPlaceOpoParams, OrderListPlaceOpoPendingSideEnum,
        OrderListPlaceOpoPendingTypeEnum, OrderListPlaceOpoWorkingSideEnum,
...
async fn main() -> Result<()> {
    ...
    let response = connection
        .order_list_place_opo(params)
        .await
        .context("order_list_place_opo request failed")?;

    info!(?response.rate_limits, "order_list_place_opo rate limits");
    let data = response.data()?;
    info!(?data, "order_list_place_opo data");
