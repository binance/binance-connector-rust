...
use binance_sdk::spot::{SpotWsApi, websocket_api::BlockTradesHistoricalParams};
...
async fn main() -> Result<()> {
    ...
    let response = connection
        .block_trades_historical(params)
        .await
        .context("block_trades_historical request failed")?;

    info!(?response.rate_limits, "block_trades_historical rate limits");
    let data = response.data()?;
    info!(?data, "block_trades_historical data");
