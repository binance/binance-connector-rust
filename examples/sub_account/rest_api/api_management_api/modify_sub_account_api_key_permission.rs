...
use binance_sdk::sub_account::{
    SubAccountRestApi, rest_api::ModifySubAccountApiKeyPermissionParams,
};
...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("modify_sub_account_api_key_permission request failed")?;

    info!(?response.rate_limits, "modify_sub_account_api_key_permission rate limits");
    let data = response.data().await?;
    info!(?data, "modify_sub_account_api_key_permission data");
