...
async fn main() -> Result<()> {
    ...
    let params = QueryAlgoOrderParams::default();

    // Make the API call
    let response = rest_client
        .query_algo_order(params)
        .await
        .context("query_algo_order request failed")?;

    info!(?response.rate_limits, "query_algo_order rate limits");
    let data = response.data().await?;
    info!(?data, "query_algo_order data");
