...
use binance_sdk::derivatives_trading_options::{
    DerivativesTradingOptionsWsStreams, websocket_streams::Hour24TickerParams,
};
...
async fn main() -> Result<()> {
    ...
    let params = Hour24TickerParams::builder("btcusdt".to_string()).build()?;

    // Subscribe to the stream
    let stream = connection
        .hour24_ticker(params)
        .await
