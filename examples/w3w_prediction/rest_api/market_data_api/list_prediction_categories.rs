...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("list_prediction_categories request failed")?;

    info!(?response.rate_limits, "list_prediction_categories rate limits");
    let data = response.data().await?;
    info!(?data, "list_prediction_categories data");
