...
use binance_sdk::w3w_prediction::{
    ...
    rest_api::{CreateInboundTransferAccountTypeEnum, CreateInboundTransferParams},
};
...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("create_inbound_transfer request failed")?;

    info!(?response.rate_limits, "create_inbound_transfer rate limits");
    let data = response.data().await?;
    info!(?data, "create_inbound_transfer data");
