# Binance Rust W3W Prediction Connector
...
Enable the `w3w_prediction` feature in your `Cargo.toml` under `binance-sdk`:

```toml
[dependencies]
binance-sdk = { version = "1.0.0", features = ["w3w_prediction"] }
```

In your code, import the `w3w_prediction` client:
...
- **Official Binance W3W Prediction API docs:** [Binance API Documentation](https://developers.binance.com/docs/w3w_prediction)
...
The `w3w_prediction` module can be configured with the following options via the `ConfigurationRestApi` builder pattern:
...
To run the tests for the W3W Prediction module:

```bash
cargo test --features w3w_prediction
```
...
Please ensure that all tests pass with `--features w3w_prediction` if you're making a direct contribution. Submit a pull request only after discussing and confirming the change.
