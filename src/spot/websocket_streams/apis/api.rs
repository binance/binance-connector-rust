...
#[allow(non_camel_case_types)]
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum DiffBookDepthUpdateSpeedEnum {
    #[serde(rename = "100ms")]
    UpdateSpeed100ms,
}

impl DiffBookDepthUpdateSpeedEnum {
    #[must_use]
    pub fn as_str(&self) -> &'static str {
        match self {
...
impl std::str::FromStr for DiffBookDepthUpdateSpeedEnum {
    type Err = Box<dyn std::error::Error + Send + Sync>;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            "100ms" => Ok(Self::UpdateSpeed100ms),
            other => Err(format!("invalid DiffBookDepthUpdateSpeedEnum: {}", other).into()),
        }
    }
}
...
#[allow(non_camel_case_types)]
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum PartialBookDepthUpdateSpeedEnum {
    #[serde(rename = "100ms")]
    UpdateSpeed100ms,
}

impl PartialBookDepthUpdateSpeedEnum {
    #[must_use]
    pub fn as_str(&self) -> &'static str {
        match self {
            Self::UpdateSpeed100ms => "100ms",
...
impl std::str::FromStr for PartialBookDepthUpdateSpeedEnum {
    type Err = Box<dyn std::error::Error + Send + Sync>;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            "100ms" => Ok(Self::UpdateSpeed100ms),
            other => Err(format!("invalid PartialBookDepthUpdateSpeedEnum: {}", other).into()),
        }
    }
}
...
pub struct DiffBookDepthParams {
    ...
    pub update_speed: Option<DiffBookDepthUpdateSpeedEnum>,
}
...
pub struct PartialBookDepthParams {
    ...
    #[builder(setter(into), default)]
    #[serde(rename = "updateSpeed", default)]
    pub update_speed: Option<PartialBookDepthUpdateSpeedEnum>,
}
...
mod tests {
    ...
    fn block_trade_should_handle_incoming_message() {
        TOKIO_SHARED_RT.block_on(async {
            ...
            let payload: Value = serde_json::from_str(r#"{"e":"blockTrade","E":1772506983582,"s":"BNBBTC","t":582,"p":"0.052","q":"5838","T":1772506983321,"m":true}"#).unwrap_or_else(|_| serde_json::json!({}));
            let msg = json!({
                "stream": stream,
                "data": payload,
            });
    ...
    fn diff_book_depth_should_handle_incoming_message() {
        TOKIO_SHARED_RT.block_on(async {
            ...
            let payload: Value = serde_json::from_str(r#"{"e":"depthUpdate","E":1672515782136,"s":"BNBBTC","U":157,"u":160,"b":[["0.0024","10"]],"a":[["0.0026","100"]]}"#).unwrap_or_else(|_| serde_json::json!({}));
            let msg = json!({
                "stream": stream,
                "data": payload,
            });
