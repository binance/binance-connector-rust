...
This module provides the official Rust client for Binance's Stocks Trading API, enabling developers to interact programmatically with Binance Stocks. The library provides tools to access trade U.S.-listed stocks and ETFs programmatically through a secure and developer-friendly API.
...
Enable the `stocks` feature in your `Cargo.toml` under `binance-sdk`:

```toml
[dependencies]
binance-sdk = { version = "1.0.0", features = ["stocks"] }
```

In your code, import the `stocks` client:
...
- **Official Binance Stocks API docs:** [Binance API Documentation](https://developers.binance.com/docs/stocks)
...
The `stocks` module can be configured with the following options via the `ConfigurationRestApi` builder pattern:
...
The `stocks` module can be configured with the following options via the `ConfigurationWebsocketStreams` builder pattern:
...
To run the tests for the Stocks module:

```bash
cargo test --features stocks
```
...
Please ensure that all tests pass with `--features stocks` if you're making a direct contribution. Submit a pull request only after discussing and confirming the change.
